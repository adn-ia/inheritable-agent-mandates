...
/**
 * @title MandateGateV3 — MandateGateV2 + époque d'émission et timelock d'autorisation
...
contract MandateGateV3 {
    ...
    /// Émetteurs schnorr reconnus, par clé publique x-only.
    mapping(uint256 => bool) public authorizedIssuerKeys;

    /// Époque d'émission : l'instant où le contrat a reconnu cette clé. C'est le
    /// contrat qui l'écrit, pas l'émetteur — d'où le plafond de durée qui en découle.
    mapping(uint256 => uint64) public issuerEpoch;

    /// Propositions d'autorisation en attente (timelock).
    mapping(uint256 => uint64) public proposedAt;

    /// Durée de vie maximale d'un verdict, comptée depuis l'époque d'émission.
    uint64 public constant MAX_WINDOW = 7 days;

    /// Délai entre la proposition d'une clé et son autorisation effective.
    uint64 public constant TIMELOCK = 2 days;

    event IssuerKeyProposed(uint256 indexed issuerKey, uint64 proposedAt);
    event IssuerKeySet(uint256 indexed issuerKey, bool allowed, uint64 epoch);
    event IssuerEpochRefreshed(uint256 indexed issuerKey, uint64 epoch);
    ...
    /**
     * Autoriser une clé se fait en DEUX temps. Un gardien seul, s'il est compromis,
     * ne peut pas introduire un émetteur complaisant dans l'instant : la proposition
     * est publique et le délai laisse le temps de la voir et de la couper.
     */
    function proposeIssuerKey(uint256 issuerKey) external onlyGuardian {
        proposedAt[issuerKey] = uint64(block.timestamp);
        emit IssuerKeyProposed(issuerKey, uint64(block.timestamp));
    }

    function confirmIssuerKey(uint256 issuerKey) external onlyGuardian {
        uint64 p = proposedAt[issuerKey];
        require(p != 0, "not proposed");
        require(block.timestamp >= uint256(p) + TIMELOCK, "timelock not elapsed");
        proposedAt[issuerKey] = 0;
        authorizedIssuerKeys[issuerKey] = true;
        issuerEpoch[issuerKey] = uint64(block.timestamp);
        emit IssuerKeySet(issuerKey, true, uint64(block.timestamp));
    }

    /**
     * Révoquer est IMMÉDIAT, et annule aussi toute proposition en cours — sinon un
     * gardien compromis pourrait proposer, se faire révoquer, puis confirmer plus tard.
     * Asymétrie assumée : accorder est lent, couper est instantané.
     */
    function revokeIssuerKey(uint256 issuerKey) external onlyGuardian {
        authorizedIssuerKeys[issuerKey] = false;
        issuerEpoch[issuerKey] = 0;
        proposedAt[issuerKey] = 0;
        emit IssuerKeySet(issuerKey, false, 0);
    }

    /**
     * Ré-horodate une clé DÉJÀ autorisée. Immédiat, sans timelock : cette opération
     * ne peut qu'invalider en masse les verdicts liés à l'ancienne époque, jamais
     * accorder un pouvoir nouveau. C'est le levier de rotation.
     */
    function refreshIssuerEpoch(uint256 issuerKey) external onlyGuardian {
        require(authorizedIssuerKeys[issuerKey], "issuer key not authorized");
        issuerEpoch[issuerKey] = uint64(block.timestamp);
        emit IssuerEpochRefreshed(issuerKey, uint64(block.timestamp));
    }
        ...
        returns (bytes32 c)
    {
        ...
        // --- époque d'émission : plafonne la durée de vie depuis un instant que le
        //     CONTRAT a écrit, pas depuis une échéance que l'émetteur se choisit ---
        require(m.epoch == issuerEpoch[m.issuerKey], "stale issuer epoch");
        require(
            _at(m.preimage, m.offEpoch,
                abi.encodePacked("\\\"issuer_epoch\\\":", _dec(uint256(m.epoch)))),
            "epoch not bound to verdict"
        );
